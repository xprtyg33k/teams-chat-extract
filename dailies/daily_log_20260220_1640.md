...
## Later updates (2026-02-20 17:xx UTC)

Implemented follow-up UX and backend changes: sidebar reorder, history "View" (load results into grid), per-row chat actions (copy/export), server-side Graph filtering (`$filter`, `$expand`, `$top`), Export date defaults + shortcut buttons, and reduced auth/polling frequency. Unit tests: 67/67 passing.
